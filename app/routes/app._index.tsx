...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (values.metaEnabled && !values.metaPixelId) {
    fieldErrors.metaPixelId = "Pixel ID is required when Meta tracking is enabled.";
  }
...
function FieldError({ message }: { message?: string }) {
  ...
  return (
    <p style={{ margin: "6px 0 0", color: "#b42318", fontSize: 13 }}>{message}</p>
  );
}
...
export default function Index() {
  ...
  useEffect(() => {
    if (fetcher.data?.ok) {
      shopify.toast.show("Meta settings saved");
    }
  ...
  return (
    <s-page heading="Meta Pixel Settings">
      <s-section heading="Store connection">
        <s-paragraph>
          Save your Meta Pixel ID here and this app will configure the COD flow
          automatically for <strong>{shop}</strong>.
        </s-paragraph>
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          <div style={{ fontSize: 14, color: "#111827" }}>
            COD order details:{" "}
            <strong>IP, user agent, _fbp, _fbc, event ID and UTM fields stored</strong>
          </div>
        </div>
      </s-section>
